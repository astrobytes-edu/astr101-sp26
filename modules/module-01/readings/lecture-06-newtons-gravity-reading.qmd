...
**Connection to Lecture 5:** This reading directly answers the "why" questions from Kepler's laws. The transition from empirical → physical is the key conceptual shift.
:::
...
2. **Universal Gravitation:** $F = GMm/r^2$ — same equation works for apples, moons, and planets. "Inverse-square" means double distance → 1/4 the force.
...
::: {.callout-note title="FIGURE PLACEHOLDER: Newton's Cannon" collapse="false"}
**Figure ID:** `newtons-cannon`

**Description:** Classic Newton's cannon thought experiment. A cannon on top of a very tall mountain with multiple trajectories showing: (1) Low speed → projectile curves and hits Earth, (2) Medium speed → projectile curves more, still hits Earth, (3) Orbital speed → projectile "falls around" Earth continuously, (4) Escape speed → projectile escapes to infinity. Caption: "The Moon is simply a cannonball moving fast enough to keep missing Earth as it falls."

**Alt text:** Newton's cannon fires projectiles at increasing speeds; at orbital velocity, the projectile falls continuously around Earth.
:::
...
**Constant speed does NOT mean constant velocity.** Direction changes → velocity changes → acceleration.
:::
...
::: {.callout-note title="FIGURE PLACEHOLDER: Acceleration Includes Direction Changes" collapse="false"}
**Figure ID:** `acceleration-direction-change`

**Description:** Show a ball moving in a circle at constant speed. Velocity vectors at several points (always tangent to circle). Show that velocity vector changes direction. Show acceleration vector pointing toward center. Caption: "Even at constant speed, the changing direction means changing velocity — which means acceleration."

**Alt text:** A ball moves in a circle with velocity arrows tangent to the path; the change in velocity direction shows there is acceleration toward the center.
:::
...
::: {.callout-note title="FIGURE PLACEHOLDER: Centripetal Force Examples" collapse="false"}
**Figure ID:** `centripetal-force-examples`

**Description:** Three examples side by side: (1) Planet orbiting Sun — gravity provides centripetal force, (2) Ball on string swung in circle — tension provides centripetal force, (3) Car turning on road — friction provides centripetal force. Each shows force arrow pointing toward center.

**Alt text:** Three examples showing different forces (gravity, tension, friction) all providing the centripetal force for circular motion.
:::
...
::: {.callout-tip title="Solution" collapse="true"}
**C) Exactly equal in magnitude but opposite in direction.** Newton's Third Law is absolute: every force has an equal and opposite reaction. Earth pulls on the Moon with exactly the same force that the Moon pulls on Earth. (The *accelerations* differ because the masses differ, but the *forces* are equal.) This is why Earth has tides — the Moon's pull is real and measurable!
:::
...
::: {.callout-note title="FIGURE PLACEHOLDER: Gravity Equation Visualized" collapse="false"}
**Figure ID:** `gravity-equation-visual`

**Description:** Show two masses $m_1$ and $m_2$ separated by distance $r$. Force arrows pointing toward each other (both feel the same force). Label all quantities. Perhaps show what happens when you double $r$ (force becomes 1/4).

**Alt text:** Two masses attract each other with equal and opposite forces; the force depends on both masses and decreases with distance squared.
:::
...
Yes — the numbers matched perfectly. Same equation, same physics, from apple to Moon.
...
::: {.callout-note title="FIGURE PLACEHOLDER: The Inverse-Square Law" collapse="false"}
**Figure ID:** `inverse-square-diagram`

**Description:** Show concentric spheres around a mass (or light source). At $r$: one unit of "intensity" per unit area. At $2r$: sphere has 4× the surface area, so 1/4 intensity per unit area. At $3r$: sphere has 9× the surface area, so 1/9 intensity per unit area.

**Alt text:** As distance from a source doubles, the same influence spreads over 4× the area, resulting in 1/4 the intensity at any point.
:::
...
::: {.callout-tip title="Solution" collapse="true"}
**C) 3,600 times weaker.** By the inverse-square law: at 60× the distance, gravitational acceleration is $1/60^2 = 1/3600$ as strong. The Moon experiences only about 0.03% of the gravitational acceleration you feel standing on Earth's surface — but that's still enough to keep it in orbit!
...
On the Moon (where $g_{Moon} \approx 1.6$ m/s²), your scale reading would be about 1/6 as much, but your mass would be unchanged.
...
::: {.callout-note title="FIGURE PLACEHOLDER: Free Fall Creates Weightlessness" collapse="false"}
**Figure ID:** `free-fall-astronaut`

**Description:** Two panels: (1) Person standing on Earth: gravity pulls down, floor pushes up. These forces balance, person feels weight. (2) Astronaut in ISS: gravity pulls down, but floor is also falling. No normal force, so no sensation of weight.

**Alt text:** On Earth, the floor's push creates the sensation of weight. In orbit, everything falls together, so there's no floor push — weightlessness.
:::
...
::: {.callout-note title="FIGURE PLACEHOLDER: Conic Sections from Gravity" collapse="false"}
**Figure ID:** `conic-sections-orbits`

**Description:** Show a central mass (Sun) with different trajectories: Circle (bound, e=0), Ellipse (bound, 0<e<1), Parabola (just escapes, e=1), Hyperbola (escapes with speed to spare, e>1). Label each with its energy condition.

**Alt text:** Objects under inverse-square gravity follow conic sections: bound objects trace ellipses or circles; escaping objects trace parabolas or hyperbolas.
:::
...
::: {.callout-note title="🔬 Deep Dive: Orbital Velocity" collapse="true"}
...
This last point is remarkable. A satellite and a space station at the same altitude orbit at the same speed, even if one is 1000× more massive.
...
3. **Newton's Law of Gravitation:** $F = GMm/r^2$ — every mass attracts every other mass, with force decreasing as distance²
...
6. **Newton explains all of Kepler's laws:**
   - Ellipses → from inverse-square central force
...
7. **Orbits reveal mass:** Measure period and distance → calculate central mass using $M = 4\pi^2 a^3/(GP^2)$
:::
...
**4. Mass vs. Weight:** Your mass is 70 kg. What is the gravitational force on you on Earth (in newtons)? On the Moon where $g \approx 1.6$ m/s²? Does your mass change?
