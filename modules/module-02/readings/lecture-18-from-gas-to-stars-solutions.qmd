...
**Cloud A is more likely to collapse.** The Jeans criterion says a cloud will collapse if its mass exceeds the Jeans mass:

$$M > M_{\rm Jeans}$$

For a given temperature and density, the Jeans mass is a fixed value. Cloud A has 10 times the mass of Cloud B. If Cloud A's mass exceeds the Jeans mass, it will collapse. Cloud B's lower mass may or may not exceed the Jeans mass — it depends on the exact numbers, but the more massive cloud is always more likely to collapse (given identical temperature and density).

**Intuition:** Gravity scales with mass; pressure is determined by temperature and density. More mass means stronger gravity. A more massive cloud is more unstable against its own gravity.
...
**Restatement:** A protostar is embedded in a dense dust cloud. The dust obscures the protostar from visible-light telescopes, but a infrared observer detects thermal emission peaking at a wavelength of $\lambda_{\rm peak} = 20$ micrometers. Using Wien's law, estimate the temperature of the dust surrounding the protostar.
...
The surrounding dust has a temperature of about **145 K**. That is cold by everyday standards, but it is warm enough to radiate strongly in the infrared, which is why embedded protostars are often discovered with infrared telescopes rather than visible-light ones.
...
**Protostar B (10 M☉) is older.** Here's why:

The Kelvin-Helmholtz timescale scales as:

$$t_{\rm KH} \sim \frac{GM^2}{RL}$$

At a given luminosity $L$, a more massive protostar must have either:
- A larger radius $R$ (since $L = 4\pi R^2 \sigma T^4$, so $R \propto \sqrt{L/T^4}$), or
- Reached a higher core temperature and advanced further toward the main sequence.

Since the two protostars have identical luminosity, the more massive one must have contracted more (reached a higher temperature) to achieve that luminosity with its greater gravitational potential. The more massive protostar is therefore further along the contraction sequence — older in the pre-main-sequence sense.

Once they reach the main sequence, Protostar B will have a much shorter lifetime because it will burn hydrogen much faster.
