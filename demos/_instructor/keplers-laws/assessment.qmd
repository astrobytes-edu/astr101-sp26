---
title: "Kepler’s Laws — Assessment Bank"
subtitle: "Clickers + short-answer + exit ticket (with demo setup + distractors)"
page-layout: article
toc: true
search: false
---

::: {.callout-note title="Navigation"}
- Instructor hub: [/demos/_instructor/](/demos/_instructor/)
- Back to this demo guide: [Guide](index.qmd)
- Student demo: [/demos/keplers-laws/](/demos/keplers-laws/)
- This demo: [Model](model.qmd) · [Activities](activities.qmd) · [Assessment](assessment.qmd) · [Backlog](backlog.qmd)
:::

::: {.callout-roadmap title="How to use this bank"}
Each item includes a **demo setup** so you can reproduce the scenario live, plus distractors tied to common misconceptions.
:::

## Clicker questions

::: {.callout-check-yourself title="Clicker 1: Fastest at perihelion or aphelion?"}
**Prompt:** In an elliptical orbit around a star, the planet’s speed is greatest at…

A. Perihelion  
B. Aphelion  
C. It’s the same everywhere  
D. It depends only on the star’s mass, not position

**Correct:** A.

**Reasoning:** Kepler’s 2nd law (equal areas in equal times) forces higher speed at smaller $r$.

**Demo setup:** Use a high‑$e$ preset, press Play, pause near perihelion/aphelion, compare velocity readout.
:::

::: {.callout-check-yourself title="Clicker 2: Kepler 3 scaling"}
**Prompt:** If you increase the semi-major axis by a factor of 4 (same star mass), the period increases by a factor of…

A. 2  
B. 4  
C. 8  
D. 16

**Correct:** C.

**Reasoning:** $P \propto a^{3/2}$, so $P$ scales as $4^{3/2}=8$.

**Demo setup:** Set $M=1$, compare $P$ at $a=1$ and $a=4$.
:::

::: {.callout-check-yourself title="Clicker 3: Vector meaning (Newton mode)"}
**Prompt:** In Newton mode, the force (acceleration) vector points…

A. Tangent to the orbit (direction of motion)  
B. Toward the star  
C. Away from the star  
D. In a fixed direction regardless of position

**Correct:** B.

**Demo setup:** Switch to Newton mode, enable vectors, drag the planet; the force vector stays radial toward the star.
:::

## Short answer

::: {.callout-problem title="Short answer 1: The ‘two pictures’ explanation"}
**Prompt (3–5 sentences):** Explain how Newton’s inverse-square gravity connects to Kepler’s laws. Your answer must mention both (i) a force pointing toward the star and (ii) a change in orbital speed with distance.

**Answer key (core idea):** Gravity provides a central force toward the star, which implies conservation of angular momentum and a constant areal sweep rate (Kepler 2). That forces the planet to move faster when it is closer (perihelion) and slower when it is farther (aphelion). The same gravitational law also yields the period–distance scaling (Kepler 3).
:::

::: {.callout-problem title="Short answer 2: Units discipline (101 vs 201)"}
**Prompt:** The demo can display velocity in km/s (101) or cm/s (201). Explain why changing display units does **not** change the physics.

**Expected:** Units are a labeling/measurement convention; the underlying relationship is the same, and values transform by fixed conversion factors.
:::

## Exit ticket (1 minute)

::: {.callout-check-yourself title="Exit ticket: One-sentence Kepler 2"}
**Prompt:** In one sentence, state Kepler’s 2nd law in your own words.

**Expected:** A line from the star to the planet sweeps out equal areas in equal times.
:::
